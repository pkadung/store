...
public class ProductRepo extends SelectRepository<Product> {

    @Override
    public List<Product> getResult(ResultSet rs) throws SQLException {
        List<Product> products = new ArrayList<>();
        while (rs.next()) {
            products.add(new Product(rs.getInt("id"), rs.getString("name"), rs.getInt("category_id"), rs.getInt("amount"), rs.getDouble("price")));
        }
        return products;
    }
